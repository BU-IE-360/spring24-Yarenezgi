...
#Overall model seems to have a some kind of trend which makes sense in terms of time of the year construction starts and on average it finishes. Thats why at first I choosed to have a t as a trend, and other variables as linear. However, its R^2 is around 4% which is not good. So, improvement will start in the next cell.
...
#Since the faiz_artırımı and istanbul_satılıkk_daire seems to follow a trend with some lags, I have added lag as 2 and checked the model1.2 with lm. It can be seen easily from the summary, R^2 is improved to 10%.  
...
#I wanted to check if lag 5 - which is choosen just for my experiment- is not effective. It even worsen the R^2. 
```R
...
```
#Lag being 5 is not good. I wanted to experiment, just for my intuiton.
...
#For the second model, I have chosen sales of tractor and trying to explain it with searches of seller of tractor, price of gas and credit of agriculture. I believe, price of gas would be a negative correlation whereas if the credits are given more to farmers and then I expect more sales of tractors. 
...
#Looking at the pairwise correlation, my instinct is corret with price of gas. Although, all the data I have is not so correlated with sales of tractor, lets try to explain with what I have.
```R
require(GGally)
...
#When I checked autocorrelations, I see some kind of trend behaviour around lag 5, so I will add t in my model. 
```R
...
#From summary, my adjusted R^2 is around 24% which is not so good, that is also expected from the beginning. 
```R
...
#However, when I add lag 5 which I realized from autocorrelation graph, R^2 seems to get better reaching to 36%. Also I see a better correlation with tarım_kredi data, so I will try to apply some power transformations such as square to see if R^2 will get better.  
...
#Looking at the new R^2, it is now worsen, I am assuming it might be due to some deleted observation which is also due to lag 5. However, still its p value is not bad so the previous model without squared tarım_kredi seems to be a better model than last one. 
...
#This model explains how TCMB reserves with searches of faiz (interest rate), Merkez bankası altın rezervi and Merkez bankası döviz rezervi and bist.
...
#Looking at the correlation values, I see some kind of nonlinearity in bist. Faiz search is expected to have such correlation with reserves of TCMB. So, I am planning to create such model using faiz and bist datas. 
...
#Now I will check autocorrelation function for residuals. There is an obvious a trend.
...
#Adding t as trend and checking R^2 seems to be a better than other models which is almost %67. Also p value is again better than other models. 
...
#I want to reduce the trend with again with 6 lags so I tried this cell however when I checked R^2 value, it shows it is worsen rather than better.
...
#Also, I wanted to check if I can make it better with adding some power transformation with bist. Since it seems to have a nonlinearity, I have tried bist^2 however, it did not get better. So my first model3 is the best without any addition of variable pairs.
